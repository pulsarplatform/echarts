...
class TreeView extends ChartView {
  ...
  _updateViewCoordSys(seriesModel, api) {
    const data = seriesModel.getData();
    const points = [];
...
function updateNode(data, dataIndex, symbolEl, group, seriesModel) {
  ...
  const focusDataIndices = focus === 'relative' ? zrUtil.concatArray(node.getAncestorsIndices(), node.getDescendantIndices()) : focus === 'ancestor' ? node.getAncestorsIndices() : focus === 'descendant' ? node.getDescendantIndices() : null;
  if (focusDataIndices) {
...
function removeNodeEdge(node, data, group, seriesModel, removeAnimationOpt) {
  ...
  const {
    source,
    sourceLayout
  } = getSourceNode(virtualRoot, node);
