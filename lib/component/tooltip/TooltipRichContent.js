...
class TooltipRichContent {
  ...
  setContent(content, markupStyleCreator, tooltipModel, borderColor, arrowPosition) {
    ...
    zrUtil.each(['backgroundColor', 'borderRadius', 'shadowColor', 'shadowBlur', 'shadowOffsetX', 'shadowOffsetY'], propName => {
      this.el.style[propName] = tooltipModel.get(propName);
    });
    zrUtil.each(['textShadowBlur', 'textShadowOffsetX', 'textShadowOffsetY'], propName => {
