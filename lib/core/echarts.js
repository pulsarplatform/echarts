  ...
  prepareView = function (ecIns, isComponent) {
    ...
    function doPrepare(model) {
      ...
      if (!view) {
        ...
        const Clazz = isComponent ? ComponentView.getClass(classType.main, classType.sub) :
        // FIXME:TS
      ...
      if (isHighDownPayload(payload)) {
        if (model instanceof SeriesModel) {
          ...
          const {
            focusSelf,
            dispatchers
          } = findComponentHighDownDispatchers(model.mainType, model.componentIndex, payload.name, ecIns._api);
    ...
    each(payloads, batchItem => {
      ...
      if (isHighDown) {
        const {
          queryOptionMap,
          mainTypeSpecified
        } = modelUtil.preParseFinder(payload);
...
export function registerAction(actionInfo, eventName, action) {
  ...
  const actionType = isObject(actionInfo) ? actionInfo.type : [actionInfo, actionInfo = {
    event: eventName
  }][0];
