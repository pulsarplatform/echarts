  ...
  for (let i = 0; i < len; i++) {
    ...
    if (coord < 0) {
      ...
      if (prevInRangeColorStop) {
        ...
        newColorStops.push(lerpStop(prevOutOfRangeColorStop, stop, 0), lerpStop(prevOutOfRangeColorStop, stop, maxSize));
      }
...
class LineView extends ChartView {
  ...
  render(seriesModel, ecModel, api) {
    ...
    const visualColor = getVisualGradient(data, coordSys, api) || data.getVisual('style')[data.getVisual('drawType')];
  ...
  _initSymbolLabelAnimation(data, coordSys, clipShape) {
    ...
    const seriesDelayValue = zrUtil.isFunction(seriesDelay) ? seriesDelay(null) : seriesDelay;
    data.eachItemGraphicEl(function (symbol, idx) {
      ...
      if (el) {
        ...
        const delay = zrUtil.isFunction(seriesDelay) ? seriesDelay(idx) : seriesDuration * ratio + seriesDelayValue;
      ...
      if (dataIndex >= 0) {
        setLabelStyle(polyline, getLabelStatesModels(seriesModel, 'endLabel'), {
          ...
          defaultText(dataIndex, opt, interpolatedValue) {
            return interpolatedValue != null ? getDefaultInterpolatedLabel(data, interpolatedValue) : getDefaultLabel(data, dataIndex);
          },
  ...
  _doUpdateAnimation(data, stackedOnPoints, coordSys, api, step, valueOrigin, connectNulls) {
    const polyline = this._polyline;
    const polygon = this._polygon;
    const seriesModel = data.hostModel;
