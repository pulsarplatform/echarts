...
const builders = {
  ...
  axisName(opt, axisModel, group, transformGroup) {
    ...
    const pos = [nameLocation === 'start' ? extent[0] - gapSignal * gap : nameLocation === 'end' ? extent[1] + gapSignal * gap : (extent[0] + extent[1]) / 2,
    // Reuse labelOffset.
...
function buildAxisLabel(group, transformGroup, axisModel, opt) {
  ...
  each(labels, function (labelItem, index) {
    ...
    const align = itemLabelModel.getShallow('align', true) || labelLayout.textAlign;
    ...
    const verticalAlign = itemLabelModel.getShallow('verticalAlign', true) || itemLabelModel.getShallow('baseline', true) || labelLayout.textVerticalAlign;
