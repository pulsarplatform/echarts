...
class SliderZoomView extends DataZoomView {
  ...
  _renderDataShadow() {
    ...
    const otherDim = candlestickDim && data.getDimensionInfo(candlestickDim) ? seriesModel.getShadowDim() // @see candlestick
    : info.otherDim;
    if (otherDim == null) {
      return;
    }
  ...
  _updateDataInfo(nonRealtime) {
    ...
    function setLabel(handleIndex) {
      ...
      const textPoint = graphic.applyTransform([orderedHandleEnds[handleIndex] + (handleIndex === 0 ? -offset : offset), this._size[1] / 2], barTransform);
